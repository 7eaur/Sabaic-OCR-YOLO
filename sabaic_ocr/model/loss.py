...
class YoloLoss(nn.Module):
    def __init__(self, anchors: Sequence[Sequence[Sequence[float]]], num_classes: int, image_size: int = 640, box_weight: float = 5.0, obj_weight: float = 1.0, cls_weight: float = 1.0, noobj_weight: float = 0.25):
        super().__init__()
    ...
    def _decode_positive_boxes(self, raw, grid_x, grid_y, h, w, anchor_wh):
    ...
    def forward(self, predictions: List[torch.Tensor], targets: List[torch.Tensor]) -> LossBreakdown:
        ...
        for b, t in enumerate(targets):
            ...
            for j in range(t.shape[0]):
                ...
                assignments[scale_idx].append((b, anchor_idx, gj, gi, cls_id, t[j, 1:5]))
        ...
        for scale_idx, items in assignments.items():
            ...
            decoded = self._decode_positive_boxes(raw_pos[:, :4], gi, gj, pred.shape[2], pred.shape[3], anchor_wh)
            ciou = bbox_ciou(decoded, gt_boxes)
            total_box = total_box + (1.0 - ciou).mean()
            if self.num_classes > 1:
                cls_target = torch.zeros((len(items), self.num_classes), dtype=raw_pos.dtype, device=device)
        ...
        return LossBreakdown(total=total, box=total_box.detach(), obj=total_obj.detach(), cls=total_cls.detach(), positives=positive_count)
